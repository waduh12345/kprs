...
import { HandshakeIcon, LandmarkIcon, Pencil, Check, X, Link as LinkIcon, ImageIcon, Palette, Upload, Image as ThemedImage, LucideIcon } from "lucide-react";
import Link from "next/link";
import { useSearchParams } from "next/navigation";
import { useState, useEffect, useRef, ChangeEvent, ElementType, ComponentPropsWithoutRef } from "react";
...
// =========================================
// 1. Komponen Helper: EditableContent
// =========================================
      ...
      <div className="flex gap-2 items-start relative z-20 my-1 font-normal">
        {multiline ? (
          <textarea 
            ...
            className={cn("w-full p-2 border-2 border-blue-500 rounded bg-white text-gray-900 shadow-lg min-h-[120px] text-base resize-y", className)} 
          ...
          <input 
            ...
            value={tempValue} 
            onChange={(e) => setTempValue(e.target.value)} 
            className={cn("w-full p-2 border-2 border-blue-500 rounded bg-white text-gray-900 shadow-lg", className)} 
...
// =========================================
// 2. Komponen Helper: EditableButton
// =========================================
const EditableButton = ({
  ...
  if (isEditMode) {
    return (
      <div onClick={() => setIsEditing(true)} role="button" className={cn(className, "relative group transition flex items-center gap-x-1.5 cursor-pointer hover:outline-2 hover:outline-dashed hover:outline-blue-500 hover:ring-4 hover:ring-blue-50/50")}>
        <Icon className="size-5 shrink-0" /><span className="truncate">{label}</span>
        <span className="absolute -top-2 -right-2 hidden group-hover:flex h-6 w-6 bg-blue-600 text-white rounded-full items-center justify-center shadow-md z-10 pointer-events-none"><Pencil size={12} /></span>
      </div>
    );
  }

  return (
    <Link href={url} className={cn(className, "relative group transition flex items-center gap-x-1.5 cursor-pointer")}>
      <Icon className="size-5 shrink-0" /><span className="truncate">{label}</span>
    </Link>
  );
};
...
export default function Hero() {
  // Hapus pemanggilan useTranslation jika tidak digunakan, atau gunakan prefix _ jika ingin diabaikan
  useTranslation({ id, en }); 
  ...
  const sectionStyle = heroContent.background.type === 'color' 
    ...
    : { backgroundImage: `url(${heroContent.background.value})`, backgroundSize: 'cover' as const, backgroundPosition: 'center' as const };
