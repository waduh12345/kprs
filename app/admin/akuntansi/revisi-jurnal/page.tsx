...
const initialDummyJurnalList: JurnalPosted[] = [
  {
    ...
    status: "Posted",
    details: [
      { coa: "510001", coa_name: "Beban Gaji", debet: 50000000, kredit: 0, keterangan_baris: "Beban gaji bruto" },
      { coa: "211001", coa_name: "Utang Gaji", debet: 0, kredit: 50000000, keterangan_baris: "Kredit ke utang gaji" },
    ],
  },
  {
    no_bukti: "JRN-1002",
    ...
    status: "Posted",
    details: [
      { coa: "111001", coa_name: "Kas di Tangan", debet: 150000, kredit: 0, keterangan_baris: "Pengisian kas" },
      { coa: "610001", coa_name: "Beban Administrasi", debet: 0, kredit: 150000, keterangan_baris: "Pengeluaran" },
    ],
  },
];
...
export default function RevisiJurnalPage() {
  ...
  const [jurnalOriginal, setJurnalOriginal] = useState<JurnalPosted | null>(null); 
  ...
  const handleSearch = () => {
    ...
    // Simulasi pencarian
    setTimeout(() => {
      const found = initialDummyJurnalList.find(d => d.no_bukti === searchQuery.trim().toUpperCase());
      
      if (found) {
        if (found.status === 'Revisi') {
            setSearchError(`Jurnal ${found.no_bukti} sudah direvisi. Cari jurnal yang belum direvisi.`);
            setJurnalOriginal(null);
        } else {
            // Simpan salinan data untuk revisi
            setJurnalOriginal(found);
            setJurnalRevisi({ ...found, details: [...found.details.map(d => ({ ...d }))] }); 
            setSearchError(null);
        }
      } else {
        setSearchError(`Nomor Bukti Jurnal ${searchQuery} tidak ditemukan atau statusnya tidak valid.`);
      }
  ...
  const handleUpdateRevisi = (index: number, field: keyof JurnalDetail, value: any) => {
    if (!jurnalRevisi) return;
    
    const newDetails = jurnalRevisi.details.map((detail, i) => {
        if (i === index) {
            // Update COA Name jika COA ID berubah
            if (field === 'coa') {
                const newCoa = dummyCOAs.find(c => c.id === value);
                return { ...detail, coa: value, coa_name: newCoa?.name || 'COA Tidak Ditemukan' };
            }
  ...
  const handlePostRevisi = async () => {
    ...
    const originalString = JSON.stringify({ deskripsi: jurnalOriginal.deskripsi, details: jurnalOriginal.details });
    const revisedString = JSON.stringify({ deskripsi: jurnalRevisi.deskripsi, details: jurnalRevisi.details });

    if (originalString === revisedString) {
        return Swal.fire("Perhatian", "Tidak ada perubahan yang dilakukan.", "info");
    }

    const { isConfirmed } = await Swal.fire({
      title: "Konfirmasi Revisi Jurnal",
      html: `
        Anda akan merevisi Jurnal **${jurnalRevisi.no_bukti}** (Total: ${formatRupiah(jurnalRevisi.total_nominal)}).
    ...
    setTimeout(() => {
      ...
      const index = initialDummyJurnalList.findIndex(d => d.no_bukti === jurnalRevisi.no_bukti);
      if (index !== -1) {
        ...
        <Card className="shadow-lg border-t-4 border-yellow-500">
          <CardHeader>
            <CardTitle className="text-xl flex items-center gap-2 text-yellow-700">
              <Edit className="h-5 w-5" /> Koreksi Jurnal **{jurnalRevisi.no_bukti}**
            </CardTitle>
            <p className="text-sm text-gray-500">
                **Perhatian:** Anda hanya dapat merevisi **Deskripsi/Keterangan** dan **Akun (COA)**. Nominal tidak dapat diubah di sini.
            </p>
          </CardHeader>
          <CardContent>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4 pb-4 border-b">
              <div>
                ...
                <p className="font-bold flex items-center gap-1"><Calendar className="h-4 w-4" /> {jurnalOriginal.tanggal}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Total Nominal</p>
                <p className="font-bold text-lg text-primary">{formatRupiah(jurnalOriginal.total_nominal)}</p>
              </div>
              <div className="col-span-1">
                <Label htmlFor="rev_deskripsi" className="text-sm text-gray-500">Revisi Deskripsi Jurnal</Label>
                <Input
                    id="rev_deskripsi"
                    value={jurnalRevisi.deskripsi}
                    onChange={(e) => setJurnalRevisi({ ...jurnalRevisi, deskripsi: e.target.value })}
                    className="mt-1"
                />
              </div>
            </div>
            ...
            <h4 className="font-semibold text-lg mb-2">Revisi Rincian Debet / Kredit:</h4>
            <div className="p-0 overflow-x-auto border rounded-lg">
                <table className="min-w-full text-sm">
                        ...
                        {jurnalRevisi.details.map((detail, index) => (
                            <tr key={index} className="border-t">
                                <td className="px-4 py-2">
                                    <select
                                        value={detail.coa}
                                        onChange={(e) => handleUpdateRevisi(index, 'coa', e.target.value)}
                                ...
                                <td className="px-4 py-2">
                                    <Input
                                        ...
                                        onChange={(e) => handleUpdateRevisi(index, 'keterangan_baris', e.target.value)}
                    ...
                    <tfoot className="bg-yellow-100 font-bold">
                         <tr>
                            <td colSpan={2} className="px-4 py-2 text-right">TOTAL NOMINAL JURNAL</td>
                            <td className="px-4 py-2 text-right">{formatRupiah(jurnalOriginal.total_nominal)}</td>
                            <td className="px-4 py-2 text-right">{formatRupiah(jurnalOriginal.total_nominal)}</td>
                        </tr>
                    </tfoot>
                </table>
      ...
      {!jurnalRevisi && !searchError && !isSearching && (
        <div className="mt-8 p-4 bg-blue-50 border-l-4 border-blue-500 text-blue-700 rounded-lg">
          ...
          <p className="text-sm mt-1">
            Gunakan fungsi ini hanya untuk mengoreksi **COA** atau **Keterangan**. Jika nominal salah, batalkan jurnal menggunakan menu **Pembatalan Jurnal** dan posting ulang dengan nilai yang benar.
          </p>
        </div>
      )}
    </div>
