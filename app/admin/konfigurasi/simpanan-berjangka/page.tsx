...
export default function PinjamanKategoriPage() {
  const [form, setForm] = useState<Partial<SimpananBerjangka>>({
    status: 1,
    nominal: 0,
  });
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        ...
        Swal.fire("Sukses", "Produk Simpanan Berjangka diperbarui", "success");
      } else {
        await create(payload).unwrap();
        Swal.fire("Sukses", "Produk Simpanan Berjangka ditambahkan", "success");
      }
  ...
  const handleDelete = async (item: SimpananBerjangka) => {
    const confirm = await Swal.fire({
      title: "Yakin hapus produk simpanan berjangka?",
      text: item.name,
      icon: "warning",
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        await refetch();
        Swal.fire("Berhasil", "Produk Simpanan Berjangka dihapus", "success");
      } catch (error) {
        Swal.fire("Gagal", "Gagal menghapus produk simpanan berjangka", "error");
        console.error(error);
      }
    }
  };
      ...
      <div className="rounded-md bg-white p-4 border border-gray-100 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="w-full flex flex-col gap-3 sm:flex-row sm:items-center">
            <Input
              placeholder="Cari produk simpanan berjangka..."
              value={query}
              onChange={(e) => {
          ...
          <div className="shrink-0 flex flex-wrap items-center gap-2">
            ...
            {openModal && <Button onClick={openModal}><Plus /> Produk Simpanan Berjangka</Button>}
          </div>
        </div>
      </div>
