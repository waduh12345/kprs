...
import LocationForm from "@/components/form-modal/asset/lokasi-form";
...
export default function AssetLocationPage() {
  ...
  const handleDetail = (item: Location) => {
    setSelectedData(item);
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen font-sans">
        ...
        <div className="flex items-center justify-between border-t border-gray-100 bg-gray-50 px-6 py-4">
          <p className="text-sm text-gray-600 font-medium">
            ...
            <span className="font-bold text-gray-900">
              {locationData?.currentPage || 1}
            </span>{" "}
            dari{" "}
            <span className="font-bold text-gray-900">
              {locationData?.pageTotal || 1}
            </span>
          </p>
          <div className="flex gap-2">
            ...
            <button
              onClick={() =>
                setPage((prev) =>
                  Math.min(prev + 1, locationData?.pageTotal || 1)
                )
              }
              disabled={page === (locationData?.pageTotal || 1)}
              className="rounded-md border border-gray-300 bg-white px-3 py-1 text-sm font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Berikutnya
            </button>
      ...
      {/* Modal Form dengan props isDetail */}
      <LocationForm
        isOpen={isFormOpen}
