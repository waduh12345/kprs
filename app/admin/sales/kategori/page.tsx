...
} from "@/services/admin/sales/kategori.service";
import { Kategori } from "@/types/admin/sales/kategori";
import FormKategori from "@/components/form-modal/admin/sales/kategori-form";
...
export default function PinjamanKategoriPage() {
  ...
  const [updateCategory, { isLoading: isUpdating }] =
    useUpdateKategoriMutation();
  const [deleteCategory] = useDeleteKategoriMutation();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        ...
        Swal.fire("Sukses", "Kategori Sales diperbarui", "success");
      } else {
        await createCategory(payload).unwrap();
        Swal.fire("Sukses", "Kategori Sales ditambahkan", "success");
      }
  ...
  const handleDelete = async (item: Kategori) => {
    const confirm = await Swal.fire({
      title: "Yakin hapus Kategori Sales?",
      text: item.name,
      icon: "warning",
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        await refetch();
        Swal.fire("Berhasil", "Kategori Sales dihapus", "success");
      } catch (error) {
        Swal.fire("Gagal", "Gagal menghapus Kategori Sales", "error");
        console.error(error);
      }
    }
  };
      ...
      <div className="rounded-md bg-white p-4 border border-gray-100 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="shrink-0 flex flex-wrap items-center gap-2">
            ...
            {openModal && <Button onClick={openModal}><Plus /> Kategori Sales</Button>}
          </div>
        </div>
      </div>
